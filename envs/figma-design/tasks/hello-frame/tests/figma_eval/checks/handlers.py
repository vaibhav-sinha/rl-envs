...
def _run_node_exists(spec: dict[str, Any], after: Envelope) -> SubCheckResult:
    node_id = spec["node_id"]
    ok = node_exists(after, node_id)
    return _check_result(spec, 1.0 if ok else 0.0, True, {"node_id": node_id, "exists": ok})
...
def run_spec_check(
    ...
    if check_type == "node_exists":
        return _run_node_exists(spec, after)
