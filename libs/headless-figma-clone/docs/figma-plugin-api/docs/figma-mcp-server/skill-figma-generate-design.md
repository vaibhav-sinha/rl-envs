<!-- source: https://developers.figma.com/docs/figma-mcp-server/skill-figma-generate-design -->

- Figma MCP Server
- Agent skills
- Skill: Build / Update Screens and Views from Design System
...
## Skill Boundaries[​](#skill-boundaries "Direct link to Skill Boundaries")
...
## Parallel Workflow with generate\_figma\_design (Web Apps Only)[​](#parallel-workflow-with-generate_figma_design-web-apps-only "Direct link to Parallel Workflow with generate_figma_design (Web Apps Only)")
...
1. **In parallel:**
   - Start building the screen using this skill's workflow (use\_figma + design system components)
   - Run `generate_figma_design` to capture a pixel-perfect screenshot of the running web app
2. **Once both complete:** Update the use\_figma output to match the pixel-perfect layout from the `generate_figma_design` capture. The capture provides the exact spacing, sizing, and visual treatment to aim for, while your use\_figma output has proper component instances linked to the design system. If the capture contains images, transfer them to your use\_figma output by copying `imageHash` values from the capture's image fills (see Step 5 for details).
3. **Once confirmed looking good:** Delete the `generate_figma_design` output — it was only used as a visual reference.

This combines the best of both: `generate_figma_design` gives pixel-perfect layout accuracy, while use\_figma gives proper design system component instances that stay linked and updatable.
...
## Required Workflow[​](#required-workflow "Direct link to Required Workflow")
...
### Step 1: Understand the Deliverable[​](#step-1-understand-the-deliverable "Direct link to Step 1: Understand the Deliverable")
...
4. **Check whether the view contains any images** (e.g., `<img>`, `<Image>`, background images, product photos, avatars, icons loaded from URLs). If it does and this is a web app, you **must** run the parallel `generate_figma_design` capture workflow — start it immediately alongside Step 2 so the capture runs while you discover components. See "Parallel Workflow with generate\_figma\_design" above.

### Step 2: Collect Component Keys, Variables, and Styles[​](#step-2-collect-component-keys-variables-and-styles "Direct link to Step 2: Collect Component Keys, Variables, and Styles")
...
#### 2a: Discover components[​](#2a-discover-components "Direct link to 2a: Discover components")
...
#### 2b: Discover variables (colors, spacing, radii)[​](#2b-discover-variables-colors-spacing-radii "Direct link to 2b: Discover variables (colors, spacing, radii)")
...
#### 2c: Discover styles (text styles, effect styles)[​](#2c-discover-styles-text-styles-effect-styles "Direct link to 2c: Discover styles (text styles, effect styles)")
...
### Step 3: Create the Wrapper Frame First[​](#step-3-create-the-wrapper-frame-first "Direct link to Step 3: Create the Wrapper Frame First")
...
### Step 4: Build Each Section Inside the Wrapper[​](#step-4-build-each-section-inside-the-wrapper "Direct link to Step 4: Build Each Section Inside the Wrapper")
...
#### Override instance text with setProperties()[​](#override-instance-text-with-setproperties "Direct link to Override instance text with setProperties()")
...
#### Read source code defaults carefully[​](#read-source-code-defaults-carefully "Direct link to Read source code defaults carefully")
...
#### What to build manually vs. import from design system[​](#what-to-build-manually-vs-import-from-design-system "Direct link to What to build manually vs. import from design system")
...
### Step 5: Validate the Full View and Transfer Images[​](#step-5-validate-the-full-view-and-transfer-images "Direct link to Step 5: Validate the Full View and Transfer Images")
...
#### Transfer images from the generate\_figma\_design capture[​](#transfer-images-from-the-generate_figma_design-capture "Direct link to Transfer images from the generate_figma_design capture")
...
1. Find all image nodes in the capture output by searching for fills with `type === "IMAGE"`:
   ...
   return imageNodes;
   ```
2. Match each captured image to the corresponding frame in your use\_figma output (by position, name, or order).
...
### Step 6: Updating an Existing View[​](#step-6-updating-an-existing-view "Direct link to Step 6: Updating an Existing View")
...
Skill: Code Connect](skill-figma-code-connect.md)[Next

Skill: create\_new\_file — Create a New Figma File](skill-figma-create-new-file.md)

- [Skill Boundaries](#skill-boundaries)
- [Prerequisites](#prerequisites)
- [Parallel Workflow with generate\_figma\_design (Web Apps Only)](#parallel-workflow-with-generate_figma_design-web-apps-only)
- [Required Workflow](#required-workflow)
  - [Step 1: Understand the Deliverable](#step-1-understand-the-deliverable)
  - [Step 2: Collect Component Keys, Variables, and Styles](#step-2-collect-component-keys-variables-and-styles)
  - [Step 3: Create the Wrapper Frame First](#step-3-create-the-wrapper-frame-first)
  - [Step 4: Build Each Section Inside the Wrapper](#step-4-build-each-section-inside-the-wrapper)
  - [Step 5: Validate the Full View and Transfer Images](#step-5-validate-the-full-view-and-transfer-images)
  - [Step 6: Updating an Existing View](#step-6-updating-an-existing-view)
