<!-- source: https://developers.figma.com/docs/figma-mcp-server/skill-figma-code-connect -->
...
## Step 1: Parse the Figma URL[​](#step-1-parse-the-figma-url "Direct link to Step 1: Parse the Figma URL")
...
## Step 2: Discover Unmapped Components[​](#step-2-discover-unmapped-components "Direct link to Step 2: Discover Unmapped Components")
...
## Step 3: Fetch Component Properties[​](#step-3-fetch-component-properties "Direct link to Step 3: Fetch Component Properties")
...
- **INSTANCE\_SWAP** — swappable nested instances tied to a specific component (icon, avatar)
- **SLOT** — flexible content regions (freeform layout, mixed children); use `getSlot()` in templates (not the same as INSTANCE\_SWAP)

Save this property list — you will use it in Step 5 to write the template.

## Step 4: Identify the Code Component[​](#step-4-identify-the-code-component "Direct link to Step 4: Identify the Code Component")
...
## Step 5: Create the Template File (.figma.ts)[​](#step-5-create-the-template-file-figmats "Direct link to Step 5: Create the Template File (.figma.ts)")
...
### Property mapping[​](#property-mapping "Direct link to Property mapping")
...
| INSTANCE\_SWAP | `instance.getInstanceSwap('Name')` | Swapped instance for a fixed component slot (then `hasCodeConnect()` / `executeTemplate()`) - do not confuse with the SLOT property below |
...
### Exhaustive variant handling[​](#exhaustive-variant-handling "Direct link to Exhaustive variant handling")
...
**INSTANCE\_SWAP** — access swappable component instances:
...
**SLOT** — `getSlot(propName)` is only valid when the Figma component property reported in Step 3 has type **`SLOT`**. Do not use `getSlot()` for **INSTANCE\_SWAP** properties (those use `getInstanceSwap()`). Slots are explicit “content regions” in the component definition, not generic nested instances.
...
### Interpolation in tagged templates[​](#interpolation-in-tagged-templates "Direct link to Interpolation in tagged templates")
...
### Finding descendant layers[​](#finding-descendant-layers "Direct link to Finding descendant layers")

When you need to access children that aren't exposed as component properties:

| Method | Use when |
| --- | --- |
| `instance.getInstanceSwap('PropName')` | Figma property type is **INSTANCE\_SWAP** (fixed swapped instance) |
...
### Nested configurable instances[​](#nested-configurable-instances "Direct link to Nested configurable instances")

A component may contain child instances that are **not exposed as component properties** (no INSTANCE\_SWAP) but are still **independently configurable** — they have their own variants, properties, or swap slots. These must be resolved dynamically, not hardcoded.
...
### Nested component example[​](#nested-component-example "Direct link to Nested component example")
...
### Conditional props[​](#conditional-props "Direct link to Conditional props")
...
## Step 6: Validate[​](#step-6-validate "Direct link to Step 6: Validate")
...
- **No hardcoded children** — verify that every INSTANCE\_SWAP property and child component slot uses the dynamic APIs (`getInstanceSwap()`, `findInstance()`, `findConnectedInstance()`, etc.) with `executeTemplate()`. No slot should contain hardcoded component content.
...
## Inline Quick Reference[​](#inline-quick-reference "Direct link to Inline Quick Reference")
...
### InstanceHandle Methods[​](#instancehandle-methods "Direct link to InstanceHandle Methods")
...
### TextHandle Properties[​](#texthandle-properties "Direct link to TextHandle Properties")
...
### SelectorOptions[​](#selectoroptions "Direct link to SelectorOptions")
...
### Export Structure[​](#export-structure "Direct link to Export Structure")
...
## Rules and Pitfalls[​](#rules-and-pitfalls "Direct link to Rules and Pitfalls")
...
5. **Use `getSlot()` only when the Figma property type is `SLOT`.** For **INSTANCE\_SWAP** props, use `getInstanceSwap()` (returns an `InstanceHandle`). `getSlot()` returns structured slot sections, not instances — never call `executeTemplate()` on its return value.
...
## Complete Worked Example[​](#complete-worked-example "Direct link to Complete Worked Example")
...
## Additional Reference[​](#additional-reference "Direct link to Additional Reference")
...
Avoid selecting large, heavy frames](avoid-large-frames.md)[Next

Skill: Build / Update Screens and Views from Design System](skill-figma-generate-design.md)

- [Overview](#overview)
- [Prerequisites](#prerequisites)
- [Step 1: Parse the Figma URL](#step-1-parse-the-figma-url)
- [Step 2: Discover Unmapped Components](#step-2-discover-unmapped-components)
- [Step 3: Fetch Component Properties](#step-3-fetch-component-properties)
- [Step 4: Identify the Code Component](#step-4-identify-the-code-component)
- [Step 5: Create the Template File (.figma.ts)](#step-5-create-the-template-file-figmats)
  - [File location](#file-location)
  - [Template structure](#template-structure)
  - [Property mapping](#property-mapping)
  - [Exhaustive variant handling](#exhaustive-variant-handling)
  - [Interpolation in tagged templates](#interpolation-in-tagged-templates)
  - [Finding descendant layers](#finding-descendant-layers)
  - [Nested configurable instances](#nested-configurable-instances)
  - [Nested component example](#nested-component-example)
  - [Conditional props](#conditional-props)
- [Step 6: Validate](#step-6-validate)
- [Inline Quick Reference](#inline-quick-reference)
  - [`instance.*` Methods](#instance-methods)
  - [InstanceHandle Methods](#instancehandle-methods)
  - [TextHandle Properties](#texthandle-properties)
