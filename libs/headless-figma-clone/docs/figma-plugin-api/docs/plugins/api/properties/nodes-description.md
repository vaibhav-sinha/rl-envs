<!-- source: https://developers.figma.com/docs/plugins/api/properties/nodes-description -->

- Plugins
- [Shared Node Properties](../node-properties.md)
- description

On this page

The plain-text annotation entered by the user for this style/component.

Supported on:

- [ComponentNode](../ComponentNode.md)
- [ComponentSetNode](../ComponentSetNode.md)
- [EffectStyle](../EffectStyle.md)
- [GridStyle](../GridStyle.md)
- [PaintStyle](../PaintStyle.md)
- [TextStyle](../TextStyle.md)

## Signature[​](#signature "Direct link to Signature")

### [description](nodes-description.md): string

## Remarks[​](#remarks "Direct link to Remarks")

To set a rich-text description using markdown, see [`descriptionMarkdown`](nodes-descriptionmarkdown.md)

caution

⚠️ There is a currently a bug in Figma where the description field will appear to be missing or not up to date. Until this is fixed, the workaround is to re-publish nodes for which the description is missing.

[Previous

deleteDevResourceAsync](nodes-deletedevresourceasync.md)[Next

descriptionMarkdown](nodes-descriptionmarkdown.md)

- [Signature](#signature)
- [Remarks](#remarks)
