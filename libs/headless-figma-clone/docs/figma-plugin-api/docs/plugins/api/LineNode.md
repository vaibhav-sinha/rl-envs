<!-- source: https://developers.figma.com/docs/plugins/api/LineNode -->
...
The line node represents a one-dimensional object. It is typically given a stroke so that it is visible. Its two primary properties are its length and rotation. In Figma and in our API, this is represented by the `width` and `rotation`. The height of a line is always 0. Lines can be created using [`figma.createLine`](properties/figma-createline.md).

## Line properties[​](#line-properties "Direct link to Line properties")
...
InteractiveSlideElementNode](InteractiveSlideElementNode.md)[Next
