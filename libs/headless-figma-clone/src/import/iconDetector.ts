import type { SerializedNode } from './snapshotSchema.js';

/** Container types that can be exported as a single SVG icon. */
...
export function boundsFromSerializedNode(node: SerializedNode): { width: number; height: number } {
...
/** Aggregate structural stats for a serialized subtree (node included). */
...
/**
 * True when this node is a compact vector-art icon (mask/boolean/vector structure only).
 * Does not use names, whitelists, or page conventions.
 */
export function isStructuralIconExportRoot(node: SerializedNode, analysis?: IconTreeAnalysis): boolean {
  ...
  // Mask-group icons: vector/boolean art or image-masked rectangle silhouettes.
  if (a.hasMaskCluster) {
...
/**
 * Figma node ids to export via `exportAsync({ format: 'SVG' })`.
 * When a parent and child both qualify, keeps the outermost (shallowest) root only.
 */
...
/** Stamp `hfcIconSvgAsset` on serialized nodes selected for SVG export. */
