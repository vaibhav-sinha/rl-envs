...
import { computeAbsoluteBoundingBox } from '../geometry/coordinates.js';
...
export interface MetadataNodeDTO {
  ...
  type: string;
  name: string;
  /** Page-absolute bounding box (Plugin API absoluteBoundingBox semantics). */
...
function applyAbsoluteBounds(dto: MetadataNodeDTO, nodeId: string, graph: GraphIndexes): void {
  const box = computeAbsoluteBoundingBox(nodeId, graph);
  if (box) dto.bounds = box;
}

function applyShapeBoxMetadata(
  dto: MetadataNodeDTO,
  node: { id: string; effects?: Effect[]; blendMode?: string; visible?: boolean; opacity?: number; rotation?: number },
  graph: GraphIndexes
): void {
  applyAbsoluteBounds(dto, node.id, graph);
...
function metadataChildNodes(node: AnyTreeNode, graph: GraphIndexes): AnyTreeNode[] | undefined {
  ...
  if (node.type === 'COMPONENT') {
    ...
    const frame = graph.nodes.get(comp.rootFrameId);
  ...
  if (node.type === 'COMPONENT_SET') {
    ...
    for (const cid of set.componentIds) {
      const comp = graph.nodes.get(cid);
...
export function collectMetadataTree(
  root: AnyTreeNode,
  options: { maxDepth?: number; signal?: AbortSignal; working?: FileEnvelope; graph?: GraphIndexes } = {}
): MetadataNodeDTO {
  const max = options.maxDepth ?? 1_000_000;
  const signal = options.signal;
  const graph =
    options.graph ?? (options.working ? buildGraphIndexes(options.working) : undefined);
  let metaSteps = 0;
  ...
  function walk(node: AnyTreeNode, depth: number): MetadataNodeDTO {
    ...
    if (graph) {
      if (node.type === 'FRAME') {
        ...
        applyAbsoluteBounds(dto, f.id, graph);
      ...
      if (node.type === 'TEXT') {
        ...
        applyAbsoluteBounds(dto, t.id, graph);
      ...
      if (
        ...
        const s = node as { id: string; effects?: Effect[]; blendMode?: string };
        applyAbsoluteBounds(dto, s.id, graph);
      ...
      if (node.type === 'SECTION') {
        applyShapeBoxMetadata(dto, node as SectionNode, graph);
      }
      if (node.type === 'GROUP') {
        applyShapeBoxMetadata(dto, node as GroupNode, graph);
      }
      if (node.type === 'TRANSFORM_GROUP') {
        applyShapeBoxMetadata(dto, node as TransformGroupNode, graph);
      }
      if (node.type === 'BOOLEAN_OPERATION') {
        const b = node as BooleanOperationNode;
        applyAbsoluteBounds(dto, b.id, graph);
      ...
      if (node.type === 'TABLE') {
        ...
        applyAbsoluteBounds(dto, tb.id, graph);
      ...
      if (node.type === 'COMPONENT_INSTANCE') {
        ...
        applyAbsoluteBounds(dto, ci.id, graph);
      ...
      if (node.type === 'INSTANCE') {
        ...
        applyAbsoluteBounds(dto, inst.id, graph);
    ...
    const kids = graph ? metadataChildNodes(node, graph) : undefined;
    if (kids !== undefined) {
