...
describe('ExportStreamSessionStore.finish', () => {
  let baseDir: string;
  let config: ReturnType<typeof loadConfig>;
  let store: ExportStreamSessionStore;
  ...
  beforeEach(() => {
    ...
    config = loadConfig();
    store = new ExportStreamSessionStore(config);

    globalThis.fetch = vi.fn(async (input: RequestInfo | URL) => {
      const url = typeof input === 'string' ? input : input.toString();
      if (url.includes('/import/hfc')) {
        return new Response(
          JSON.stringify({
            fileKey: 'fk',
            fileName: 'Minimal',
  ...
  it('assembles NDJSON parts and writes standalone HFC via mocked import', async () => {
    const snapshot = JSON.parse(
      ...
      document: { id: string; type: string; name: string; properties: Record<string, unknown>; children?: unknown[] };
    ...
    for (const line of lines) {
      store.appendPart(exportId, line);
    }
    ...
    const importCall = fetchMock.mock.calls.find((c) => String(c[0]).includes('/import/hfc'));
    expect(importCall).toBeDefined();
    const body = JSON.parse((importCall![1] as RequestInit).body as string) as {
      assetFiles?: unknown[];
      snapshot: { assets: unknown[] };
    };
    expect(body.snapshot.assets).toEqual([]);
    expect(body.assetFiles ?? []).toEqual([]);
  });
});
